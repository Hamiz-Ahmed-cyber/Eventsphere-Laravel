...
<form method="POST" action="{{ route('admin.announcements.store') }}" class="card max-w-2xl space-y-5">@csrf
    <div><label class="block text-sm font-medium">Title</label><input required name="title" value="{{ old('title') }}" class="admin-announcement-title mt-1 w-full rounded-lg"></div>
